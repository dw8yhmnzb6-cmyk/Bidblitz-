  ...
  return (
    <div 
      className={`bg-gradient-to-b from-gray-800/80 to-gray-900/80 rounded-xl border ${
        probability >= 70 ? 'border-green-500/50' : 
        probability >= 50 ? 'border-cyan-500/30' : 
        'border-gray-700/50'
      } overflow-hidden hover:shadow-lg transition-all`}
    >
      {/* Header with probability */}
      <div className={`p-3 ${
        probability >= 70 ? 'bg-green-500/10' : 
        probability >= 50 ? 'bg-cyan-500/10' : 
        'bg-gray-800/50'
      }`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Brain className={`w-5 h-5 ${probability >= 50 ? 'text-green-400' : 'text-gray-400'}`} />
            <span className={`font-bold ${probability >= 50 ? 'text-green-400' : 'text-gray-400'}`}>
              {probability}% {t.winProbability}
            </span>
          </div>
          <span className={`text-xs px-2 py-1 rounded-full ${
            opportunity.confidence === 'high' ? 'bg-green-500/20 text-green-400' :
            opportunity.confidence === 'medium' ? 'bg-yellow-500/20 text-yellow-400' :
      ...
      <div className="p-4">
        <div className="flex gap-3 mb-3">
          {/* Product Image */}
          <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center flex-shrink-0 cursor-pointer"
          ...
          <div className="flex-1 min-w-0">
            <h3 
              className="text-white font-bold text-sm line-clamp-2 cursor-pointer hover:text-cyan-400"
              onClick={() => navigate(`/auctions/${opportunity.auction_id}`)}
            >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="bg-gray-800/50 rounded-lg p-2 text-center">
            ...
            <p className="text-white text-sm font-bold">{formatTime(timeLeft)}</p>
            <p className="text-gray-500 text-[10px]">{t.timeLeft}</p>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-2 text-center">
            <Users className="w-4 h-4 text-gray-400 mx-auto mb-1" />
            <p className="text-white text-sm font-bold">{opportunity.total_bids || 0}</p>
            <p className="text-gray-500 text-[10px]">{t.bidders}</p>
          </div>
          <div className="bg-gray-800/50 rounded-lg p-2 text-center">
            <BarChart3 className="w-4 h-4 text-gray-400 mx-auto mb-1" />
            <p className={`text-sm font-bold ${isGoodChance ? 'text-green-400' : 'text-yellow-400'}`}>
        ...
        <Button 
          ...
          className={`w-full ${
            probability >= 70 ? 'bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400' :
            probability >= 50 ? 'bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-400 hover:to-blue-400' :
...
export default function AIBidRecommendationsPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black py-8 px-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Bitte anmelden für KI-Empfehlungen</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black py-8 px-4" data-testid="ai-bid-page">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-500/20 border border-purple-500/30 mb-4">
            <Brain className="w-5 h-5 text-purple-400" />
            <span className="text-purple-400 font-bold">KI-POWERED</span>
          </div>
